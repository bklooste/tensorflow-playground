...
@Component({
  selector: 'my-tensor-ui',
  templateUrl: './tensorui.component.html',
  styleUrls: ['./tensorui.component.css']
})
export class Player {
...
function makeGUI() {
  ...
  let showTestData = d3.select('#show-test-data').on('change', () => {
  ...
  const noise = d3.select('#noise').on('input', () => {
  ...
  const batchSize = d3.select('#batchSize').on('input', () => {
  ...
  const activationDropdown = d3.select('#activations').on('change', () => {
  ...
  const learningRate = d3.select('#learningRate').on('change', () => {
  ...
  const regularRate = d3.select('#regularRate').on('change', () => {
  ...
  const problem = d3.select('#problem').on('change', () => {
...
function updateWeightsUI(network: nn.Node[][], container: d3.Selection<d3.BaseType, {}, HTMLElement, any>) {
...
function drawNode(cx: number, cy: number, nodeId: string, isInput: boolean,
  ...
  if (isInput) {
    const label = INPUTS[nodeId].label != null ?
  ...
  if (!isInput) {
    ...
    nodeGroup.append('rect')
      ...
      .on('mouseenter', function() {
        updateHoverCard(HoverType.BIAS, node, d3.mouse(container.node() as any));
  ...
  const nodeHeatMap = new HeatMap(RECT_SIZE, DENSITY / 10, xDomain,
...
function drawNetwork(network: nn.Node[][]): void {
  ...
  for (let layerIdx = 1; layerIdx < numLayers - 1; layerIdx++) {
    ...
    for (let i = 0; i < numNodes; i++) {
      ...
      for (let j = 0; j < node.inputLinks.length; j++) {
        const link = node.inputLinks[j];
        const path: SVGPathElement = drawLink(link, node2coord, network,
...
function getRelativeHeight(selection: d3.Selection<d3.BaseType, {}, HTMLElement, any>) {
...
function updateDecisionBoundary(network: nn.Node[][], firstTime: boolean) {
  ...
  const xScale = d3.scaleLinear().domain([0, DENSITY - 1]).range(xDomain);
  const yScale = d3.scaleLinear().domain([DENSITY - 1, 0]).range(xDomain);
